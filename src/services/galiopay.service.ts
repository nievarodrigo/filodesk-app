...
// CVU y Alias de servidor, nunca expone al cliente
...
export async function createPaymentLink(input: CreatePaymentLinkInput) {
  ...
  const isSandbox = process.env.NODE_ENV !== 'production' || process.env.GALIOPAY_SANDBOX === 'true'

  const notificationUrl = isDev && process.env.NGROK_URL
    ? `${process.env.NGROK_URL}/api/webhooks/galiopay`
    : `${siteUrl}/api/webhooks/galiopay`
